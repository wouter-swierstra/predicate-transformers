...
open import PropSpec

-- We will work with a state that contains a single natural number.
-- However, to pass the initial state to the postcondition we let propositions depend on two natural numbers.
prop : Set₁
prop = Nat → Nat → Set

instance
  PropState : Propositional prop
  Propositional.forAll PropState C P n n'
    = (n'' : Nat) (c : C) → P c n' n''
  Propositional._=>_ PropState P Q n n'
    = P n n' → Q n n'
  Propositional._∧_ PropState P Q n n'
    = Pair (P n n') (Q n n')
  Propositional.prove PropState P
    = (n : Nat) (n' : Nat) → P n n'
  Propositional.prove-forAll PropState C P
    = iff (λ x c n n' → x n n n' c) λ x n n' n'' c → x c n' n''
  Propositional.implies-reflexive PropState P n n' pf = pf
  Propositional.implies-transitive PropState P Q R P→Q Q→R n n' x
    = Q→R n n' (P→Q n n' x)
  Propositional.lemma-sharpen PropState preI postI pf n n' n'' c (fst , snd)
    = pf c n' n'' (preI n' n'' fst , λ n''' c₁ x → snd n''' c₁ (postI n n'' n''' c₁ x))
  Propositional.lemma-return PropState b pre post P y pf n n' (fst , snd)
    = {!snd n'!}
...
R : C -> Set
R Tick = ⊤
Count = PropSpec (Nat → Set) C R

spec' : {a : Set} (pre : Nat → Set) (post : Nat → a → Nat → Set) →
  Count a
spec' pre post = {!spec pre !}

tick : Count ⊤
tick = Step Tick return

ptCount : PTs (Nat → Set) C R
ptCount = {!!}

{-
-- We want to express this as a theorem on wpProp,
...
ptAndBind : {b c : Set} -> {pre : Nat -> Set} {int : b -> Nat -> Set} {post : c -> Nat -> Set} ->
  ...
  ((n : Nat) -> pre n -> wpProp ptCount int mx n) ->
  ((y : b) (n' : Nat) -> int y n' -> wpProp ptCount post (f y) n') ->
  (n : Nat) -> pre n -> wpProp ptCount post (mx >>= f) n
...
refineBind mx f x = refinement (λ P x₁ z → ptAndBind mx f (λ n z₁ → z₁) (λ y n' z₁ → z₁) x₁ (Refine.proof x (λ _ z₁ → wpProp ptCount (P x₁) (f z₁)) x₁ (Pair.fst z , Pair.snd z)))
...
-- OnPred P n holds when n has a predecessor and P holds for that predecessor.
...
rep : Nat -> PropSpec Nat C R ⊤ -> PropSpec Nat C R ⊤
